...
# "lm1" es una estructura con características de lista, así que contiene muchos objetos internos que pueden ser de varias clases. 
# Usando el operador "$" se puede acceder a los mismos, aunque también se pueden usar los paréntesis rectos dobles.
# Por ejemplo, para saber cuales son los parámetros del modelo, se puede ejecutar el siguiente comando:
lm1$coefficients # ó
lm1[[1]]
# Son dos formas idénticas de pedir lo mismo, dado que el primer elemento de la lista [[1]], son los coeficientes.

# Usando la función "names" se pueden ver los nombres de los objetos incluidos (sólo a 1 nivel de profundidad):
names(lm1)
#  [1] "coefficients"  "residuals"     "effects"       "rank"         
#  [5] "fitted.values" "assign"        "qr"            "df.residual"  
#  [9] "xlevels"       "call"          "terms"         "model"

# Algunos de estos objetos tienen complejas series de "atributos", a los cuales se puede acceder usando la función "attributes":
...
# El argumento 'subset' se utiliza para realizar dicho filtro, en este caso hace la regresión pero sólo con las observaciones tales que dist > 20
...
# Ahora si... un poco de interpretación de la salidad (output) de R:
summary(lm1)

# Este modelo presenta una tasa de cambio(pendiente) que nos dice que por cada unidad de X que me muevo hacia la derecha, aumenta de manera significativa 3.9324 unidades de Y.
# El intercepto cuyo valor es -17.5791, nos dice cuanto vale Y cuando la pendiente es cero.
...
# Para finalizar queremos aclarar que la idea de este curso no es enseñar estadística, para eso existen un montón de cursos sobre la materia dictada por docentes entendidos sobre el tema. Sin embargo, como R es un software para realizar análsis estadísticos, por ese motivo no queríamos dejar de lado las interpretaciones mínimas de los análsis que pretendemos mostrar. Para mayor profundidad existie una gran diversidad de libros sobre R y Estadística, los cuales estan a su disposición por parte nuestra si así lo desan.
