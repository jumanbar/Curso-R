...
# A su vez, usando dicho nombre se puede obtener el valor correspondiente
...
# Si recordamos, una matriz es básicamente un vector arreglado en dos
# dimensiones. Probemos a nombrar los 5 primeros elementos:
names(y) <- c("a", "b", "c", "d", "e")
...
# Estas funciones, al igual que "names", tienen la doble función de asignación y
...
# Puedo nombrar los elementos del vector de valores propios, en este caso
# numerándolos. Debe recordarse: mediante la especificación del nombre o índice
...
# observaciones de una determinada variable. Los data.frames son un tipo
# particular de listas, que además tienen algunas propiedades de las matrices.
# La primera son sus dimensiones, debido a lo cual podemos renombrar filas y
...
# elementos que componen un objeto. El uso de nombres muchas veces es
# deseable, ya que la numeración requiere de una gran memoria y no siempre se
# mantiene constante (por ejemplo, si elmininamos/agregamos elementos al
# objeto). Otras veces es estrictamente necesario, como probablemente vamos a
# ver más adelante en el curso.
