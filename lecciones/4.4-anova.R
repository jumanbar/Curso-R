...
# El Análisis de Varianza o ANOVA por sus siglas e inglés, es un modelo estadístico que evalua si las medias de distintos grupos son iguales. Su versión más simple es aquella en la cual dos grupos distintos son comparados considerando solo los efectos fijos de los mismos. Este caso particular es llamado comunmente Test de t.
# El modelo estadístico clásico del análisis de varianza de efecto fijo es el sigueinte (no correr):
y_ij = mu + a_i + e_ij

# donde y_ij es la observación j del grupo i (nótese que cada grupo es un nivel de un factor o 'tratamiento'), 'mu' es la media global poblacional, 'a_i' es la media de grupo i y e_ij es el error aleatorio correspondiente a esta observacións.
# Cada 'a_i' evalua el efecto del i-ésimno nivel de mi factor. Este se puede definir como la diferencia entre la media poblacional 'mu' y la media de cada grupo i 'mu_i' (no correr):
a_i = mu - mu_i.

# Para el caso del anova, la hipótesis nula del modelo estadístico es que las medias de cada uno de los grupos i son iguales, es decir todos tienen el mismo efecto. Es decir (no correr):
H_0: mu = mu_1 = mu_2 = ... = mu_i = ... = mu_k    # k = número de niveles

# Por lo tanto con que una de las medias sea diferente de las del resto ya se estaría rechazando la hipótesis nula.
# Atención: sólo con un ANOVA no es posible conocer cual o cuales medias son diferentes entre si, para eso se deben realizar los llamados test a posterirori y realizar correcciones del tipo Bonferroni o sus derivadas.
...
# Como dijimos el anova evalua diferencias entre grupos, por ese motivo la fórmula del modelo contiene una variable de respuesta continua y la variable explicativa con los distintos grupos i.
# POr ejemplo:
anova1 <- aov(len ~ supp, data=ToothGrowth)
# # El objeto 'anova1' realiza un análisis de varianza utilizando las variables 'len' y 'supp' contenidas en el data.frame 'ToothGrowth' (para más información usar '?ToothGrowth')
...
# Donde 'y' es la variable de respuesta con distribución Normal(0, sigma2) y 'grp' es la variable explicativa, en este caso son los distintos grupos, por lo tanto es una variable categórica o factor.
...
# El intercepto del modelo hace referencia a uno de los grupos evaluados por el modelo, en este caso el grupo 'OJ'. El valor del coeficiente es la media de dicho grupo. En este caso 'OJ' estaría siendo como la media global mu, en nuestro modelo genérico arriba, es el grupo de referncia. R utiliza un criterio alfabético para elegir al grupo de referencia, por este motivo en este caso es 'OJ'. 
# El otro coeficiente es el a_i, es decir, es la diferencia entre mu y mu_i, la media global y la media del grupo i, en este caso la media del grupo 'VC'.
# Para conocer cual es la media del grupo 'VC' se le resta a la media del grupo 'OJ'(20.66) el valor de a_i(3.7), esto nos da un valor de 16.96.

# Otra forma de conocer las medias de mis dos grupos sin tener que realizar cuentas es haciendo que el modelo no tenga intercepto:
anova2 <- aov(len ~ supp - 1, data=ToothGrowth)

coef(anova2) # Observe que aqui utilice otro comando para obteber los coeficientes de mi modelo
...
# b_0 corresponde a la media global o grupo de referencia, al igual que mu en un modelo anova y X es la matriz con las variables como columnas (siendo la primer columna compuesta por una repetición de unos, que multiplican a b_0).
# Cada 'b_k' es el parámetro a estimar para el grupo k y se define como la diferencia entre la media poblacional y la media del dicho grupo (en otras palabras b_i = a_i para todo i >= 0).
