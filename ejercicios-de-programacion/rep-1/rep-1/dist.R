...
cat.ad <- for(i in 1:length(coorx)){
  punt.x[i]<-(coorx[i]-0.5)
}

cat.op <- for(j in 1:length(coory)){
  punt.y[j]<-(coory[j]-0.5)
}

dst <- hipot(punt.x, punt.y)

i <- which.min(dst)

j <- which.max(dst)

A <- c(min(coorx[i]), min(coory[i]))

B <- c(max(coorx[j]), max(coory[j]))
